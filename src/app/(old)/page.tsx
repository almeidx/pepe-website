import { type FeaturedGuild, FeaturedGuilds } from "@/app/(old)/featured-guilds";
		...
		<div className="mb-8 flex flex-col items-center gap-10 py-4">
			<div className="flex flex-col items-center gap-4 xl:gap-0">
				...
				<div className="flex gap-5">
					...
					<Link
						className="hidden w-72 items-center justify-center gap-5 rounded-lg border border-lrk-white px-3 py-2 font-bold text-3xl transition-colors hover:bg-lrk-white hover:text-black xl:flex"
						href="/guilds"
					>
						Dashboard
						<Launch />
			...
			<div className="flex w-full items-center justify-center border-lrk-white/50 border-t border-b">
				<div className="grid w-full max-w-xl grid-cols-4 place-items-center px-4 py-2 md:px-8 xl:max-w-7xl xl:py-6">
					<div className="flex flex-col xl:flex-row xl:gap-5">
						...
						<p className="text-lrk-white/50 text-xs xl:place-self-end xl:text-base">servers</p>
					</div>
					<div className="flex flex-col xl:flex-row xl:gap-5">
						<span className="font-extrabold text-2xl text-shadow-regular xl:text-6xl">
							{formatNumber(memberCount, false)}
						</span>
						<p className="text-lrk-white/50 text-xs xl:place-self-end xl:text-base">members</p>
					</div>
					<div className="flex flex-col xl:flex-row xl:gap-5">
						<span className="font-extrabold text-2xl text-shadow-regular xl:text-6xl">{formatNumber(uptime)}%</span>
						<p className="text-lrk-white/50 text-xs xl:place-self-end xl:text-base">uptime</p>
					</div>
					<div className="flex flex-col xl:flex-row xl:gap-5">
						<span className="font-extrabold text-2xl text-shadow-regular xl:text-6xl">
							{formatNumber(messageCount, false)}
						</span>
						<p className="text-lrk-white/50 text-xs xl:place-self-end xl:text-base">messages</p>
					</div>
				</div>
			</div>
...
function StartLevelingButton() {
	return (
		<ExternalLink
			...
			href={BOT_INVITE}
		>
			<div className="size-full rounded-lg py-2 transition-colors group-hover:bg-lrk-background">
