...
import type { GuildInfo } from "@/app/(old)/guilds/page.tsx";
			...
			>
				{guilds.map((guild) => (
					<MenuItem key={guild.id} store={menu}>
						<Link
							className="flex cursor-pointer items-center gap-2 px-1.5 py-2 hover:bg-lrk-white/10"
			...
			<div>
				<Link
					className={clsx(
						...
						guild.premium
							? "border border-lrk-white text-lrk-white"
...
function Item({ guildId, name, path, isActive, children }: ItemProps) {
	return (
		<li>
			<Link
				className={clsx(
					"flex items-center gap-7 rounded-xl py-2 pr-5 pl-3 transition-colors hover:bg-lrk-white/10",
					isActive && "bg-lrk-white/10",
				)}
