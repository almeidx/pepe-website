	...
	return (
		<>
			<div className="flex flex-wrap items-center gap-3">
				...
				<SelectLabel className="text-lg text-lrk-white/75 tracking-tight md:text-xl" store={select}>
				...
				>
					{Object.entries(BadgeInfo).map(([flag, { icon, name }]) => (
						<SelectItem
							...
							className="flex cursor-pointer items-center gap-2 text-lg text-lrk-white/75 tracking-tight hover:text-lrk-white md:text-xl"
							store={select}
							value={flag}
						>
				...
				<button
					...
					type="button"
				>
					<PersonAdd className="size-6 text-lrk-white" />
				</button>
			</div>
...
function OnJoinRoleBadge({ flagId, id, premium, onDelete, roleIds, roles }: OnJoinRoleBadgeProps) {
	...
	return (
		<div className="flex items-center gap-4">
			<button
				className="group relative flex size-10 items-center justify-center rounded-lg border border-lrk-white bg-[#1e1f22] text-[#fff] text-lg md:text-xl"
				onClick={() => onDelete(id)}
				type="button"
			>
				<div className="absolute hidden size-10 items-center justify-center rounded-lg border border-lrk-white bg-[#1e1f22] group-hover:flex">
