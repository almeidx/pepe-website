	...
	return (
		<div className="flex flex-col gap-2 rounded-lg">
			...
			>
				<SelectItem
					key="none"
					className="flex cursor-pointer items-center gap-2 text-lg text-lrk-white/75 tracking-tight hover:text-lrk-white"
					store={select}
					value=""
					disabled
					hidden
				...
				{bots.map(({ name, icon }) => (
					<SelectItem
						key={name}
						className="flex cursor-default items-center gap-2 text-lg text-lrk-white/75 tracking-tight data-[active-item]:text-lrk-white"
						store={select}
						value={name}
					>
