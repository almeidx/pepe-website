...
import type { GuildInfo } from "@/app/(old)/levels/page";
		...
		<>
			...
			<div className="my-7 flex max-w-2xl flex-wrap justify-center gap-12">
				{filteredGuilds.map((guild, idx) => (
					<Link
						className="group relative flex size-20 items-center justify-center rounded-lg border border-lrk-white/25 bg-[#1e1f22]"
						href={`/levels/${guild.vanity ?? guild.id}`}
						key={guild.id}
						prefetch={false}
					>
						<div
							className="-left-11 -top-14 invisible absolute z-50 w-40 rounded-lg bg-darker px-3 py-2 text-lrk-white shadow-md outline outline-1 outline-lrk-white/25 group-hover:visible"
							role="tooltip"
						>
...
export function LeaderboardGuildInput() {
	...
	return (
		<>
			...
			<div className="mt-6 flex flex-col items-center gap-2 text-center text-lrk-white/75 text-xl tracking-tight">
