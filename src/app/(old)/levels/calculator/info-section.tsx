...
export function InfoSection({ children, title, tooltip, raw }: InfoSectionProps) {
	...
	return (
		<div className="relative flex h-24 w-64 flex-col items-center justify-between rounded-lg border border-lrk-white/25 bg-dark-gray py-2">
			<h2 className="text-center text-lrk-white/75 text-xl tracking-tighter">{title}</h2>
			...
			<Tooltip
				store={tooltipStore}
				className="max-w-xs rounded-lg border border-lrk-white/25 bg-darker p-2 leading-relaxed tracking-tight md:max-w-prose"
			>
				{tooltip}
			</Tooltip>
		</div>
