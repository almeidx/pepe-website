...
import { userLevelResetAction } from "@/app/(old)/levels/[entry]/actions";
import type { GetLevelsResponse } from "@/app/(old)/levels/[entry]/page";
...
export function LeaderboardTableRow({ guildId, row, isManager }: LeaderboardTableRowProps) {
	...
	return (
		<>
			<Disclosure
				className={clsx(
					"flex w-full items-center gap-1 border border-lrk-white/25 bg-dark-gray px-2 py-1.5 hover:bg-dark-gray/50",
			...
			<DisclosureContent
				className="mb-4 flex w-full flex-col justify-between gap-2 rounded-b-lg border-lrk-white/25 border-x border-b bg-dark-gray/50 px-2 py-1.5 md:flex-row md:gap-0"
				store={disclosure}
			>
				<div>
					<p>
						Experience: <span className="text-lrk-white/75">{row.xp.toLocaleString("en")}</span>
					</p>
					<p>
						Experience until next level:{" "}
						<span className="text-lrk-white/75">{(row.nextLevelXp - row.xp).toLocaleString("en")}</span>
					</p>
					<p>
						Progress: <span className="text-lrk-white/75">{row.progress.toFixed(1)}%</span>
					</p>
					<p>
						Messages: <span className="text-lrk-white/75">{row.messageCount.toLocaleString("en")}</span>
					</p>
				</div>
