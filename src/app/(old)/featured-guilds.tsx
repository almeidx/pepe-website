...
export function FeaturedGuilds({ guilds, direction = "left", speed }: FeaturedGuildsProps) {
	return (
		<div className="relative">
			...
			<div className="absolute inset-y-0 right-[90%] left-0 z-10 bg-gradient-to-l from-transparent to-lrk-background" />
			<div className="absolute inset-y-0 right-0 left-[90%] z-10 bg-gradient-to-r from-transparent to-lrk-background" />
		</div>
	);
}
...
function FeaturedGuild({ id, icon, name, memberCount, partner, verified, index }: FeaturedGuild & { index: number }) {
	...
	return (
		<div className="flex select-none items-center gap-4 rounded-lg border border-lrk-white/25 bg-darker px-3 py-2 md:gap-6">
			<ImageWithFallback
				alt={`${name}'s icon`}
			...
			<div className="flex flex-col">
				...
				<p className="text-lrk-white/50 text-md md:text-lg">
